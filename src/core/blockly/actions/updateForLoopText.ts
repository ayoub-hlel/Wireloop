...
const updateForLoopText = (blockEvent: BlockEvent): ForLoopTextChange[] => {
  ...
  return forBlocks.map((block) => {
    ...
    if (isEmpty(toInput!.blockId) || isEmpty(fromInput!.blockId)) {
      return {
        blockId: block.id,
        changeText: "by",
    ...
    const toField = toBlock!.fieldValues.find((field) => field.name == "NUM");
    const fromField = fromBlock!.fieldValues.find(
    ...
    const toNumber = +toField.value;
    const fromNumber = +fromField.value;
